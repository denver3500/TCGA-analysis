...
output_dir <- "TCGA-Chaperones/WGCNA_by_clusters_test/processed_data"
log_file <- "TCGA-Chaperones/WGCNA_by_clusters_test/normalization_log.txt"
...
# Process only test projects
message("Processing test projects: ", paste(test_projects, collapse = ", "))

# Check which projects have cluster data
available_projects <- c()
for (project_id in test_projects) {
  cluster_file <- file.path(cluster_dir, project_id, paste0(project_id, "_patient_clusters.csv"))
  if (file.exists(cluster_file)) {
    available_projects <- c(available_projects, project_id)
  } else {
    message("Warning: Cluster file not found for ", project_id)
  }
}

message("Available projects: ", paste(available_projects, collapse = ", "))

# Process each available project
all_results <- list()
...
message("Test projects processed: ", paste(names(all_results), collapse = ", "))
