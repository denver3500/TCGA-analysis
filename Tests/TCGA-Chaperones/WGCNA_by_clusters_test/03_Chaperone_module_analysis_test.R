...
output_dir <- "TCGA-Chaperones/WGCNA_by_clusters_test/chaperone_analysis"
log_file <- "TCGA-Chaperones/WGCNA_by_clusters_test/chaperone_analysis_log.txt"
...
analyze_chaperone_modules <- function(project_id, cluster_id) {
  ...
  chaperone_modules <- chaperone_modules %>%
    ...
    select("gene_id", "base_ensembl", "Name", "module_number", "module_color")
  ...
  module_counts <- chaperone_modules %>%
    ...
    summarise(
      n_chaperones = n(),
      chaperone_genes = paste(Name, collapse = ", "),
      .groups = "drop"
    ) %>%
    arrange(desc(n_chaperones))
  ...
  if (nrow(module_counts) > 1) {
    ...
    p1 <- ggplot(module_counts, aes(x = reorder(module_color, n_chaperones), y = n_chaperones)) +
    ...
    # Pie chart
    p2 <- ggplot(module_counts, aes(x = "", y = n_chaperones, fill = module_color)) +
...
message("Processing chaperone analysis for test projects: ", paste(test_projects, collapse = ", "))
...
if (length(all_results) > 0) {
  ...
  clustering_consistency <- summary_stats %>%
    ...
    summarise(
      n_clusters = n(),
      avg_chaperones_per_cluster = mean(Total_Chaperones),
      avg_dominant_percentage = mean(Dominant_Percentage),
      consistent_clustering = length(unique(Dominant_Module)) == 1,
      .groups = "drop"
    )
  ...
  heatmap_data <- summary_stats %>%
    select(Project, Cluster, Dominant_Percentage) %>%
    tidyr::pivot_wider(names_from = Cluster, values_from = Dominant_Percentage, values_fill = 0) %>%
    as.data.frame()
...
message("\nChaperone module analysis complete for test projects!")
message("Processed: ", paste(unique(sapply(all_results, function(x) x$project_id)), collapse = ", "))
