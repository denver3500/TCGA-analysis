...
# Define file paths
processed_data_dir <- "TCGA-Chaperones/WGCNA_by_clusters_test/processed_data"
results_dir <- "TCGA-Chaperones/WGCNA_by_clusters_test/results"
log_file <- "TCGA-Chaperones/WGCNA_by_clusters_test/network_construction_log.txt"
...
# Find processed data for test projects
message("Looking for processed data for test projects: ", paste(test_projects, collapse = ", "))

# Process each test project and cluster
all_results <- list()

for (project_id in test_projects) {
  message("\n=== Processing project: ", project_id, " ===")
  
  # Find cluster data files for this project
  project_data_dir <- file.path(processed_data_dir, project_id)
  
  if (!dir.exists(project_data_dir)) {
    message("  No processed data directory found for ", project_id)
    next
  }
...
message("\nWGCNA network construction complete for test projects!")
